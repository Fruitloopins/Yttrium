...
public class InGameHudMixin {
    ...
    private String modifyExperienceString(String experienceString) {
        if (YttriumClient.getConfig().tweaks.trueXP) {
            ...
            return experienceString + " (" + getTotalXP(client.player) + ")";
        }
     ...
     */
    @Unique
    private int getTotalXP(ClientPlayerEntity player) {
     ...
     */
    @Unique
    private int getXPOfLevel(int level) {
        if (level >= 32) {
     ...
     */
    @Unique
    private int getXPGainedTowardsNextLevel(int level, float progress) {
        if (level >= 31) {
            return Math.round(progress * (9 * level - 158)); // i.e. xp gained = 0.05 * 112
        }

        if (level >= 16) {
            return Math.round(progress * (5 * level - 38));
        }
